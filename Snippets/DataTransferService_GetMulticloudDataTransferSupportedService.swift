...
func sample(
  ...
  let response = try await client.getMulticloudDataTransferSupportedService(
    request: GetMulticloudDataTransferSupportedServiceRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/multicloudDataTransferSupportedServices/\(multicloudDataTransferSupportedServiceId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        multicloudDataTransferSupportedServiceId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
