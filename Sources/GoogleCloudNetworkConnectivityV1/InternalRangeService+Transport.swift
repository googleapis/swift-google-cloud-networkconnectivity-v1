...
extension Clients {
  final class InternalRangeServiceTransport: InternalRangeServiceStub {
    ...
    public func listInternalRanges(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/internalRanges"
          var query = [
    ...
    public func updateInternalRange(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleCloudGax._RoutingMatcher.pathValue(
              request.internalRange.map({ $0.name }),
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/internalRanges/"), .singleWildcard,
              ],
              fieldName: "internal_range.name")
          else {
            return nil
          }
          let path = "/v1/\(pathVariable0)"
